...
//--------------------------------------------------------------------------------------
struct SimpleVertex
{	///XMFLOAT3 Describes a 3D vector consisting of three single-precision floating-point values (X coord, Y coord, Z coord)
	XMFLOAT3 Pos;
	///XMFLOAT4 Describes a 4D vector consisting of four single-precision floating-point values (X, Y, Z, W coords)
    XMFLOAT4 Color;
};

struct ConstantBuffer
{
	///These matricies store the transformation to be used when it renders
    XMMATRIX mWorld;
    XMMATRIX mView;
...
/// Note: Shaders are short programs exectued by the GPU. Take in certain input
/// data, process it, then outpt the results to the next stage of the pipeline
...
HRESULT CCube::LoadShaders()
{
	...
	/// A pixel shader takes a pixel (fragment) as input and is run once
	/// for each pixel of a primitive that we wish to render
	ID3DBlob* pPSBlob = NULL;
	hr = CompileShaderFromFile(L"D3DVisualization.fx", "PS", "ps_4_0", &pPSBlob);
	
	///If there is a problem, see through message
	if (FAILED(hr))
	{
		MessageBox(NULL,
	...
	/// A vertex shader takes a vertex as an input, it is run once for every vertex
	/// passed to the GPU via vertex buffers
...
HRESULT CCube::InitDevice()
{
	...
	/// Create vertex buffer -> holds the vertex data
	...
	///Describes the vertex buffer object to be created
	D3D11_BUFFER_DESC bd;		

	ZeroMemory(&bd, sizeof(bd));
	bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(SimpleVertex)* 8;	//8 Verticies on a Cube
	bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bd.CPUAccessFlags = 0;

	///Describes the actual data that will be copied to the vertex buffer during creation
	...
	// Create index buffer (based on vertex buffer indicies -- make triangles that make up the cube)
	WORD indices[] =
	{
		3, 1, 0,
		2, 1, 3,
	...
	// Initialize the world matrix (For world transformations)
	m_World = XMMatrixIdentity();

	// Initialize the view matrix (For view transformations)
...
{
	// Setup the viewport
	///Note: To use the entire render target for the output
	///set top left point to (0,0) and width and height identical to render
	///target size
	D3D11_VIEWPORT vp;
...
/// Allows the resource to be bound to the graphic pipeline
/// <returns>S_OK for success, or failure code</returns>
